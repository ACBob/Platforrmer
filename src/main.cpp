...
int main()
{
    ...
    // --------------------
    platform::PLATFORM_WINDOW_REF window = platform::CreatePlatformWindow(SCR_WIDTH, SCR_HEIGHT, "Hello World");
    if (window == NULL)
    {
	...
	render::PixelShader *pixelShader = renderDevice->CreatePixelShader(fragmentShaderSource);
 
	render::ShaderPipeline *pipeline = renderDevice->CreatePipeline(vertexShader, pixelShader);

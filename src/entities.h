...
namespace entities
{
	...
	class World
	{
		...
		World( float hgrav = 0.0f, float vgrav = 9.8f )
		{
			...
			// Draw everything!
			debugDraw.SetFlags( b2Draw::e_shapeBit | b2Draw::e_aabbBit | b2Draw::e_centerOfMassBit | b2Draw::e_jointBit | b2Draw::e_pairBit );
		};
		~World()
		{
			entlist.clear();
		...
		void Update( float delta )
		{
			...
			entlist.erase( std::remove_if( entlist.begin(), entlist.end(), ShouldRemove ), entlist.end() );
